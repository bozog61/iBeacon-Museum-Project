...
- (IBAction)backButton:(id)sender {
    [self performSegueWithIdentifier:@"gecis12" sender:self];
}
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    
    if([[segue identifier] isEqualToString:@"gecis12"])
    {
        listBeaconTableViewController  *list = segue.destinationViewController;
        list.girisMinor=3361;
        
    }
    if([[segue identifier] isEqualToString:@"gecis11"])
    {
...
{
    if (kontol2==1) {
        [self performSegueWithIdentifier:@"gecis11" sender:self];
    }
}
